...
import com.restaurant.caffeinapplication.data.model.DataCouponItem
import com.restaurant.caffeinapplication.data.model.DataSubscriptionItem
import java.time.Instant
import java.time.LocalDateTime
import java.time.ZoneId
import java.time.format.DateTimeFormatter

class CouponAdapter(val models : List<DataCouponItem>?) : RecyclerView.Adapter<CouponAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val instant = Instant.parse(models!![position]?.updatedAt)
        ...
        holder.voucherName.text = models[position]?.product?.name
        holder.amountUsed.text = "${models[position]?.product?.price.toString()} 원"
    }
...
class SubscribeAdapter(val models : List<DataSubscriptionItem>?) : RecyclerView.Adapter<SubscribeAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.voucherName.text = models[position]?.subscriber?.subscription?.name
        holder.amountUsed.text = "${models[position]?.subscriber?.subscription?.product?.price.toString()} 원"
    }
