...
import com.restaurant.caffeinapplication.adapter.IncomeCouponAdapter
...
import com.restaurant.caffeinapplication.databinding.ActivityIncomeCouponBinding
...
class IncomeCouponActivity : AppCompatActivity(), Popup.PopupCallback {

    lateinit var binding : ActivityIncomeCouponBinding
    lateinit var groups : MutableList<GroupIncomeCoupon>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityIncomeCouponBinding.inflate(layoutInflater)
        ...
        binding.btnReset.setOnClickListener {
            ...
            groups = mutableListOf()
            getAllCoupon(starDate = starDateNow, endDate = endDateNow)
        }
        ...
        incomeViewModel.dataIncomeCoupon.observe(this){
            when (it.message){
                "SUCCESS" -> {
                    ...
                    for (i in it.response!!.indices) {
                        for (j in it.response[i]!!.month!!.indices) {
                            ...
                            val recyclerValueList = mutableListOf<List<DataCouponItem>?>() // List to store recycler values
                            ...
                            for (k in it.response[i]!!.month!![j]!!.date!!.indices) {
                                ...
                                recyclerValueList.add(it.response[i]!!.month!![j]!!.date!![k]!!.dataCoupon as List<DataCouponItem>?)
                            }
                            ...
                            if (year != null && month != null && dateList.isNotEmpty()) {
                                ...
                                val recyclerViewData = mutableListOf<ChildItemIncomeCoupon>()

                                for ((index, modifiedDate) in modifiedDateList.withIndex()) {
                                    recyclerViewData.add(ChildItemIncomeCoupon(modifiedDate, recyclerValueList[index]))
                                }

                                val group = GroupIncomeCoupon(currentGroupTitle,totalIncomeYearValue, modifiedDateList,incomeListModif, recyclerViewData, false)
                                groups.add(group)
                            }
                        }
                    }

                    val adapter = IncomeCouponAdapter(this, groups)
                    ...
                    adapter.setItemClickListener(object : IncomeCouponAdapter.ItemClickListener {
    ...
    private fun getAllCoupon(starDate:String? = null, endDate: String? = null){
        incomeViewModel.getAllCouponViewModel(starDate = starDate, endDate = endDate)
    }
